...
Victim HTMLRepository::findSavedVictim(const std::string name)
{
...
int HTMLRepository::getSavedPosition(const Victim& victim)
{
	std::vector<Victim> victims{};
	victims = loadHTMLData(this->filename);
...
void HTMLRepository::deleteSavedVictim(const Victim& victim)
{
	std::vector<Victim> victims{};
	victims = loadHTMLData(this->filename);
	Victim foundVictim = findSavedVictim(victim.getName());
