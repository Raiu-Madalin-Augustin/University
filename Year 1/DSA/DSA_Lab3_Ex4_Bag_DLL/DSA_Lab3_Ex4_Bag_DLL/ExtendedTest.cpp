...
void testRemoveAllOccurrences() {
    cout << "Test remove all occurrences" << endl;
    Bag b;
    for (int i = -100; i < 100; i++) {
        assert(b.removeAllOccurrences(i) == false);
    }
    assert(b.size() == 0);
    ...
    for (int i = 0; i < 100; i++) {
        ...
        if (i % 2 == 0) {
            assert(b.removeAllOccurrences(i) == 1);
        }
        else {
            assert(b.removeAllOccurrences(i) == 0);
        }
    }
    assert(b.size() == 0);
    for (int i = 0; i < 10; i++) {
    ...
    assert(b.size()==10);
    assert(b.removeAllOccurrences(7) == 10);
    assert(b.size() == 0);
    for (int i = 0; i < 100; i++) {
        b.add(9);
    }
    assert(b.size()==100);
    assert(b.removeAllOccurrences(9) == 100);
    assert(b.size()==0);
}
