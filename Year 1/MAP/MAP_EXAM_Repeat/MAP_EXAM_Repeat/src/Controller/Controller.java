...
public class Controller {
    ...
    public String getSymTable(int index) throws EmptyStackException {
        if(repo.getPrgList().size() <= 0) {
            throw new EmptyStackException("ERROR: Empty execution stack!");
        }
        return repo.getPrgList().get(index).getSymTable().toString();
    }

    public String getFileTable(int index) throws EmptyStackException {
        if(repo.getPrgList().size() <= 0) {
            throw new EmptyStackException("ERROR: Empty execution stack!");
        }
        return repo.getPrgList().get(index).getFileTable().toString();
    }

    public String getHeap(int index) throws EmptyStackException {
        if(repo.getPrgList().size() <= 0) {
            throw new EmptyStackException("ERROR: Empty execution stack!");
        }
        return repo.getPrgList().get(index).getHeapTable().toString();
    }
